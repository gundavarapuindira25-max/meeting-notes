...
Turns a meeting transcript into a structured summary, key decisions, and action items using a local LLM.
...
- Runs entirely against a local Ollama model with no API key, no per-request cost, works offline
...
Runs against a local model. Install and start it before running the backend:
